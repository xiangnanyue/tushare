...
def get_balance_sheet_by_proxy(code, ip, port):
    """
        获取某股票的历史所有时期资产负债表
    Parameters
    --------
    ...
    if code.isdigit():
        ...
        headers = getheaders()
        try:
            response = requests.get(url=ct.SINA_BALANCESHEET_URL % code,
                                    proxies=proxies,
                                    headers=headers,
                                    timeout=10)
...
def get_profit_statement_by_proxy(code, ip, port):
    """
        获取某股票的历史所有时期利润表
    Parameters
    --------
    ...
    if code.isdigit():
        ...
        headers = getheaders()
        try:
            response = requests.get(url=ct.SINA_PROFITSTATEMENT_URL % code,
                                    proxies=proxies,
                                    headers=headers,
                                    timeout=10)
            print(response.status_code)
...
def get_cash_flow_by_proxy(code, ip, port):
    """
        获取某股票的历史所有时期现金流表
    Parameters
    --------
    ...
    if code.isdigit():
        ...
        headers = getheaders()
        try:
            response = requests.get(url=ct.SINA_CASHFLOW_URL % code,
                                    proxies=proxies,
                                    headers=headers,
                                    timeout=10)
            print(response.status_code)
