...
def get_realtime_quotes(symbols=None):
    ...
    try:
        ...
        col_nb = len(ct.LIVE_DATA_COLS)
        if max([len(lis) for lis in data_list]) > col_nb:
            data_list = [row[:col_nb] for row in data_list]
        df = pd.DataFrame(data_list, columns=ct.LIVE_DATA_COLS)
        print(e)
    if 's' in df.columns:
        df = df.drop('s', axis=1)
        ...
        for symbol in symbols:
            try:
                data = get_hist_data(symbol, start=start, end=end,
                                     ...
                                     pause=pause)
                data['code'] = symbol
                df = df.append(data, ignore_index=False)  # index in get_hist_data has been set to date.
            except Exception as e:
                print(symbol, "will be ignored because its data request failed, Causes:")
                print(e)
        return df
    else:
        return None
