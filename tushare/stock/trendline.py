...
def rsi(df, n=6):
    ...
    px[px < 0] = 0
    _rsi['rsi'] = _sma(px, n) / _sma((df['close'] - df['close'].shift(1)).abs(), n) * 100
    # def tmax(x):
    #     if x < 0:
    #         x = 0
    #     return x
...
def vrsi(df, n=6):
    ...
    px[px < 0] = 0
    _vrsi['vrsi'] = _sma(px, n) / _sma((df['volume'] - df['volume'].shift(1)).abs(), n) * 100
    return _vrsi
...
def bbiboll(df, n=10, k=3):
    ...
    _bbiboll['bbi_boll'] = (_ma(df.close, 3) + _ma(df.close, 6) + _ma(df.close, 12) + _ma(df.close, 24)) / 4
    _bbiboll['bbi_md'] = _md(_bbiboll.bbi_boll, n)
    _bbiboll['bbi_upr'] = _bbiboll.bbi_boll + k * _bbiboll.bbi_md
    _bbiboll['bbi_dwn'] = _bbiboll.bbi_boll - k * _bbiboll.bbi_md
    return _bbiboll
...
def priceosc(df, n=12, m=26):
    ...
    _c['date'] = df['date']
    man = _ma(df.close, n)
    _c['price_osc'] = (man - _ma(df.close, m)) / man * 100
    return _c
...
def sma(df, n, m=1):
    ...
    df = df.fillna(0)
    b = df.ewm(min_periods=0, ignore_na=False, adjust=False, alpha=m/n).mean()
    _c = pd.DataFrame()
    _c['date'] = df['date']
    _c['sma'] = b.close
    return _c
...
def mike(df, n=12):
    ...
    _mike['mike_wr'] = typ * 2 - lv
    _mike['mike_mr'] = typ + hv - lv
    _mike['mike_sr'] = 2 * hv - lv
    _mike['mike_ws'] = typ * 2 - hv
    _mike['mike_ms'] = typ - hv + lv
    _mike['mike_ss'] = 2 * lv - hv
    return _mike
...
def vmacd(df, qn=12, sn=26, m=9):
    ...
    _vmacd = pd.DataFrame()
    _vmacd['date'] = df.date
    _vmacd['vdiff'] = _ema(df.volume, qn) - _ema(df.volume, sn)
    _vmacd['vdea'] = _ema(_vmacd['vdiff'], m)
    _vmacd['vmacd'] = (_vmacd['vdiff'] - _vmacd['vdea'])
    return _vmacd
...
def mi(df, n=12):
    ...
    _mi = pd.DataFrame()
    _mi['date'] = df.date
    _mi['mi'] = _sma(df.close - df.close.shift(n), n)
    return _mi
...
def dptb(df, n=7):
    ...
    _dptb['dptb'] = _dptb.m.rolling(n).count() / n
...
def jdqs(df, n=20):
    ...
    q[q > 0] = np.nan
    _jdrs['t'] = q
    _jdrs['jdqs'] = _jdrs.m.rolling(n).count() / _jdrs.t.rolling(n).count()
...
if __name__ == "__main__":
    ...
    data = ts.get_k_data("601138", start="2020-01-02")
    # print(data)
